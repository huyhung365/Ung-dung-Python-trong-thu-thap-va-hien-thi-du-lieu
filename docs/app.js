...
async function loadData(event) {
  ...
  $('submit').textContent='Đang lấy dữ liệu…';$('status').textContent=window.weatherPythonReady ? 'Đang kết nối Open-Meteo…' : 'Đang chuẩn bị công cụ xử lý dữ liệu… Lần mở đầu có thể mất một chút thời gian.';
  const controller=new AbortController();weatherController=controller;const timer=setTimeout(()=>controller.abort(),120000);
  try {
    const response=await weatherFetch(`/api/weather?${params}`,{signal:controller.signal});
...
$('download').addEventListener('click',async()=>{
  ...
  try {
    const response=await weatherFetch(`/api/export?id=${encodeURIComponent(selected.id)}`);
...
$('location-form').addEventListener('submit', async event => {
  ...
  try {
    const response = await weatherFetch(`/api/locations?name=${encodeURIComponent(name)}`, {signal:controller.signal});
